...
import { NavigationDict, CartDict } from '@/types/dictionary';
...
export default function Header({ lang, dict, cartDict }: HeaderProps) {
  ...
  const navLinks = [
    ...
    { name: dict.aboutUs, href: `/${lang}/quienes-somos` },
      ...
      >
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between">
            ...
            {/* Desktop Controls (Language dropdown & Cart & Contact button) */}
            <div className="hidden md:flex items-center space-x-4">
              ...
              <button
                onClick={toggleCart}
                className="relative p-2.5 rounded-lg border border-ocean-700 bg-ocean-900/50 hover:bg-ocean-800 text-white transition-all duration-200 focus:outline-none flex items-center justify-center hover:-translate-y-0.5 cursor-pointer"
                title={cartDict.title}
              >
                <svg className="w-5 h-5 text-primary-400 animate-fade-in" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-3 7h3m-3 4h3m-6-4h.01M9 16h.01" />
                </svg>
                {totalItems > 0 && (
                  <span className="absolute -top-1.5 -right-1.5 flex h-5 w-5 items-center justify-center rounded-full bg-ull-gold text-[10px] font-bold text-white ring-2 ring-ocean-950 animate-scale-up">
                    {totalItems}
                  </span>
                )}
              </button>
              ...
              <Link
                href={`/${lang}/contacto`}
                className="inline-flex items-center justify-center px-4 py-2 text-sm font-semibold rounded-lg bg-ull-gold hover:bg-ull-gold-dark text-white shadow-md hover:shadow-lg transition-all duration-200 hover:-translate-y-0.5"
              >
                {dict.requestQuote}
              </Link>
            </div>

            {/* Mobile Controls (Language switcher & Cart & Burger menu) */}
            <div className="md:hidden flex items-center space-x-3">
              ...
              <button
                onClick={toggleCart}
                className="relative p-2.5 rounded-lg border border-ocean-800 bg-ocean-900/40 text-white hover:bg-ocean-800 flex items-center justify-center cursor-pointer"
                title={cartDict.title}
              >
                ...
                {totalItems > 0 && (
                  <span className="absolute -top-1.5 -right-1.5 flex h-4.5 w-4.5 items-center justify-center rounded-full bg-ull-gold text-[9px] font-bold text-white ring-1 ring-ocean-950 animate-scale-up">
                    {totalItems}
                  </span>
                )}
              </button>
